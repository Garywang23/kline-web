...
const INITIAL_SIGNALS = {
  ...
  "huifeng": {
    ...
    "require_bid1_volume": true,
    "require_yesterday_strong": true,
    "desc": "现价涨停 + 买一有挂单 + 昨日强势（涨停/炸板）"
  },
  "linban": {
    "name": "临板预警",
    "enabled": true,
  ...
  "fenqi": {
    ...
    "desc": "昨日强势 + 开盘涨幅 ≥ 2% + 涨幅 ≥ 4% + 低点反弹 ≥ 4% + 高点回撤 > -2.5% + 站上 MA5/MA10"
  },
  "ruozhuanqiang": {
    "name": "弱转强确认",
    "enabled": true,
    ...
    "max_open_pct": 3,
    "require_above_ma5": true,
    "desc": "昨日强势 + 开盘涨幅 -3%~3% + 涨幅 ≥ 3% + 低点反弹 ≥ 3% + 高点回撤 > -2.5% + 站上 MA5"
  }
};

async function loadSignals(env) {
  try {
    const raw = await env.APP_CONFIG.get(SIGNALS_KEY, "json");
    return raw ? { ...DEFAULT_SIGNALS, ...raw } : INITIAL_SIGNALS;
  } catch {
...
const DEFAULT_CONFIG = {
  refreshSeconds: 5,
  watchlist: [
    { code: "001896", name: "豫能控股", note: "" },
    { code: "603629", name: "利通电子", note: "" },
    { code: "600396", name: "", note: "" },
    { code: "601991", name: "", note: "" },
    { code: "002498", name: "", note: "" },
    { code: "300058", name: "", note: "" },
    { code: "002929", name: "", note: "" },
    { code: "002081", name: "", note: "" },
  ],
};
...
async function loadConfig(env) {
  const raw = await env.APP_CONFIG.get(CONFIG_KEY, "json") || DEFAULT_CONFIG;
...
async function saveConfig(env, config) {
  ...
  await env.APP_CONFIG.put(CONFIG_KEY, JSON.stringify(clean));
  return clean;
}
...
export async function collectSnapshot(env) {
...
function send(body, { status = 200, type = "text/plain; charset=utf-8" } = {}) {
...
export default {
  ...
  try {
    ...
    if (url.pathname === "/api/config" && request.method === "POST") {
      ...
      return send(JSON.stringify(await saveConfig(env, config)), { type: "application/json; charset=utf-8" });
    }
    if (url.pathname === "/api/watchlist" && request.method === "POST") {
      const body = await readJson(request);
      const code = normalizeCode(body.code);
      if (!code) return send("Invalid code", { status: 400 });
      const config = await loadConfig(env);
    ...
    return send(error.stack || error.message, { status: 500 });
  }
  },
};
